...
class DRNet_wo_scm(torch.nn.Module):
    ...
    se_reduction=None, res_scale=1, bottom_kernel_size=3, pyramid=False):
        super(DRNet_wo_scm, self).__init__()
        ...
        self.feat_extract = nn.Sequential(
            ConvLayer(conv, in_channels, base_channels, kernel_size=bottom_kernel_size, stride=1, norm=None, act=act),
            ConvLayer(conv, base_channels, base_channels, kernel_size=3, stride=1, norm=norm, act=act),
            ConvLayer(conv, base_channels, base_channels, kernel_size=3, stride=1, norm=norm, act=act)
        ).to(self.device)
        ...
        self.spatial_feat_extract = nn.Sequential(
            # Upsampling Layers
            ConvLayer(conv, base_channels * 2, base_channels, kernel_size=3, stride=1, norm=norm, act=act),
            ConvLayer(conv, base_channels, base_channels, kernel_size=3, stride=1, norm=norm, act=act),
            PyramidPooling(base_channels, base_channels, scales=(4,8,16,32), ct_channels=base_channels//4),
            ConvLayer(conv, base_channels, out_channels, kernel_size=1, stride=1, norm=None, act=act)
        ).to(self.device1)
    ...
    def forward(self, x, adv_pred):
        ...
        out_enc_level2 = self.encoder_level2(inp_fam2)

        inp_fam1 = self.down2_3(out_enc_level2)
        # inp_enc_level3 = self.FAM1(inp_fam1, z4)
        out_enc_level3 = self.encoder_level3(inp_fam1)
